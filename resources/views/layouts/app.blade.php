...
<head>
    ...
    <!-- Styles -->
    @vite(['resources/css/app.css', 'resources/css/aems.css', 'resources/css/aems_respon.css', 'resources/js/app.js'])
        ...
        <div id="sidebar" class="aems-sidebar aems-sidebar-gradient flex-shrink-0">
            <div class="aems-sidebar-content p-6">
                ...
                <nav class="aems-sidebar-nav">
                    <div class="space-y-1">
                        <a href="{{ route('home') }}"
                            class="aems-nav-link {{ request()->routeIs('home') ? 'active' : '' }}">
                            <span class="aems-nav-icon">🏠</span>
                            <span>Accueil</span>
                        </a>
                        <a href="{{ route('about') }}"
                            class="aems-nav-link {{ request()->routeIs('about') ? 'active' : '' }}">
                            <span class="aems-nav-icon">ℹ️</span>
                            <span>À propos</span>
                        </a>
                        <a href="{{ route('photos') }}"
                            class="aems-nav-link {{ request()->routeIs('photos') ? 'active' : '' }}">
                            <span class="aems-nav-icon">📸</span>
                            <span>Photos</span>
                        </a>
                        <a href="{{ route('videos') }}"
                            class="aems-nav-link {{ request()->routeIs('videos') ? 'active' : '' }}">
                            <span class="aems-nav-icon">🎥</span>
                            <span>Vidéos</span>
                        </a>
                        ...
                        @auth
                            ...
                            <a href="{{ route('dashboard') }}"
                                class="aems-nav-link {{ request()->routeIs('dashboard') ? 'active' : '' }}">
                                <span class="aems-nav-icon">📊</span>
                            ...
                            @if (auth()->check() && (auth()->user()->isAdmin() || auth()->user()->isMember()))
                                <a href="{{ route('posts.index') }}"
                                    class="aems-nav-link {{ request()->routeIs('posts.*') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">📝</span>
                                    <span>Articles</span>
                                </a>
                                <a href="{{ route('events.index') }}"
                                    class="aems-nav-link {{ request()->routeIs('events.*') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">📅</span>
                                    <span>Événements</span>
                                </a>
                                <a href="{{ route('events.calendar') }}"
                                    class="aems-nav-link {{ request()->routeIs('events.calendar') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">🗓️</span>
                                    <span>Calendrier</span>
                                </a>
                                <a href="{{ route('media.index') }}"
                                    class="aems-nav-link {{ request()->routeIs('media.*') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">🎬</span>
                                    <span>Médias</span>
                                </a>
                            @endif
                            ...
                            @if (auth()->check() && auth()->user()->isAdmin())
                                ...
                                <a href="{{ route('admin.dashboard') }}"
                                    class="aems-nav-link {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">⚙️</span>
                                    <span>Administration</span>
                                </a>
                                <a href="{{ route('admin.users') }}"
                                    class="aems-nav-link {{ request()->routeIs('admin.users*') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">👥</span>
                                    <span>Utilisateurs</span>
                                </a>
                                <a href="{{ route('admin.activity-logs') }}"
                                    class="aems-nav-link {{ request()->routeIs('admin.activity-logs*') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">📊</span>
                                    <span>Logs d'activité</span>
                                </a>
                                <a href="{{ route('admin.settings') }}"
                                    class="aems-nav-link {{ request()->routeIs('admin.settings*') ? 'active' : '' }}">
                                    <span class="aems-nav-icon">🔧</span>
                ...
                <div class="border-t border-white/20 pt-4 mt-auto">
                    @auth
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="w-full text-white/70 hover:text-white text-sm py-2 px-3 rounded hover:bg-white/10 transition-colors">
                                Se déconnecter
                            </button>
                        </form>
                    @else
